...
list_of_OCP.CSVs <- 
  list.files(path = "~/student_documents/UBC/Research/pH_stat/with_a_sac_(prelim)/2022-05-11/OCPs",
             recursive = TRUE,
             pattern = "\\.csv$",
             full.names = TRUE)
...
setwd("~/student_documents/UBC/Research/pH_stat/with_a_sac_(prelim)/2022-05-11/2022_05_11")


######################     import csv w/ measurements (airsac sizes)    ######################
data <- 
  read_csv("~/student_documents/UBC/Research/pH_stat/with_a_sac_(prelim)/2022-05-11/2022-05-11, area.csv") %>%
  mutate(...1 = NULL) %>%
  mutate(Mean = NULL)
print(data)
...
OCP_V_time <- OCPs_with_calcs %>%
  ungroup() %>%
  mutate(pH = (median_V + 0.4051)/0.054) %>% #old calibration
  mutate(seconds = NULL) %>%
  mutate(V = NULL) %>%
...
rep.file.time <- rep(exp.times$file.time, each = 6)
rep.jpg_files <- rep(exp.times$jpg_files, each = 6)
rep.seconds <- rep(exp.times$seconds, each = 6)
rep.minutes_from_start <- rep(exp.times$minutes_from_start, each = 6)
...
file.reps <- data.frame(rep.file.time) %>%
  ...
  mutate(sac = rep(c(1,2,3,4,5,6), times = nrow(exp.times))) %>% #sac ID column
  group_by(sac) %>%
  mutate(
...
ggplot(data = file.reps, 
  ...
  geom_point(aes(y= (pH *20) - 154), # magnify relative to main axis, 
             #translate pH points so final point = zero -> see scale_y_continuous
             size = 4.4,
             colour = "orangered3") +
 # scale_y_continuous(sec.axis = sec_axis(~./1 + 0, name = "pH")) + # /20 to undo magnify, 
