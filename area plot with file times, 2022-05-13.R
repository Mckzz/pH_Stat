...
setwd("~/student_documents/UBC/Research/pH_stat/with_a_sac_(prelim)/2022-05-13/2022_05_13")


######################     import csv w/ measurements (airsac sizes)    ######################
data <- 
  read_csv("~/student_documents/UBC/Research/pH_stat/with_a_sac_(prelim)/2022-05-13/2022-05-13, area.csv") %>%
  mutate(...1 = NULL) %>%
  mutate(Mean = NULL)
print(data)
